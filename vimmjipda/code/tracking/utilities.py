...
def gaussian_mixture(means, covariances, weights):
    ...
    covariances = np.dot(
        weights.T,
        covariances.reshape(covariances.shape[0], covariances.shape[1] ** 2)
        + outer_product.reshape(covariances.shape[0], covariances.shape[1] ** 2),
    ).reshape(np.atleast_2d(weights).shape[0], covariances.shape[1], covariances.shape[2])
...
def ensure_min_mode_probability(min_mode_probability, mode_probabilities, tau=None):
    ...
    if any(mode_probability < min_mode_probability for mode_probability in mode_probabilities):
        indices = [
            idx for idx, mode_probability in enumerate(mode_probabilities) if mode_probability < min_mode_probability
        ]
        ...
        if any(
            mode_probability - subtract_value / (n_modes - len(indices)) < min_mode_probability
            for mode_probability in mode_probabilities[indices_inv]
        ):
...
def auction_algorithm(reward_m, epsilon=0.01):
    ...
    while unassigned_participants != []:
        ...
        prices[selected_participant[0]] = (
            prices[selected_participant[0]] + (values[-1] - values[-2]) + epsilon
        )  # epsilon ensures convergence

    for i in range(n_participants):
        reward = (
            reward + reward_m["value"][assigned_participants[i][0], assigned_participants[i][1]]
        )  # compute combined reward
        assigned_participants[i] = reward_m["index"][
            assigned_participants[i][0], assigned_participants[i][1]
        ]  # get assignment value from reward matrix
