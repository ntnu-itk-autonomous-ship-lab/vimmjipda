...
import numpy as np
import vimmjipda.code.tracking.utilities as utilities
...
class MurtyDataAssociator(DataAssociator):
    ...
    def __compute_marginal_association_probabilities__(self):
        ...
        for k, a_k in enumerate(self.__association_hypotheses__):
            for a_k_i in a_k:
                ...
                marginal_association_probabilities[assigned_track][
                    assigned_measurement
                ] += self.__association_probabilities__[k]
        return marginal_association_probabilities

    def __create_association_hypotheses__(self):

        if (
            self.__cluster__.n_tracks >= self.__n_measurements_murty__
            and self.__cluster__.n_measurements >= self.__n_tracks_murty__
        ):
    ...
    def __create_reward_matrix__(self):
        ...
        """
        reward_m = np.zeros(
            (self.__cluster__.n_measurements + self.__cluster__.n_tracks, self.__cluster__.n_tracks),
            dtype=[("value", "float"), ("index", "2int8")],
        )
        for i, estimate in enumerate(self.__cluster__.tracks):
            for j, measurement in enumerate(self.__cluster__.measurements):
                if measurement in estimate.measurements:
                    reward_m["value"][j, i] = np.log(self.__cluster__.w[i, j]) - np.log(
                        self.__cluster__.clutter_density
                    )
