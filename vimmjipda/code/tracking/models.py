...
from vimmjipda.code.tracking import constructs
...
class MeasurementModel(object):
    ...
    def predict_measurement(self, state):
        ...
        S = self.__measurement_mapping__.dot(state.covariance).dot(
            self.__measurement_mapping__.T
        ) + self.get_measurement_covariance(state.mean)
        return z_hat, S
...
class PolarMeasurementModel(MeasurementModel):
    ...
    def get_measurement_covariance(self, mean):
        ...
        current_bearing = np.arctan2(
            state_position[1] - self.__ownship_position__[1], state_position[0] - self.__ownship_position__[0]
        )
...
class CombinedMeasurementModel(CartesianMeasurementModel, PolarMeasurementModel):
    ...
    def get_measurement_covariance(self, mean):
        return PolarMeasurementModel.get_measurement_covariance(
            self, mean
        ) + CartesianMeasurementModel.get_measurement_covariance(self, mean)
...
class CTModel(KinematicModel):
    ...
    def __repr__(self):
        return "{}({:1.4f},{:1.4f})".format(
            self.__class__.__name__,
            self.__linear_state_transition_covariance__,
            self.__angular_state_transition_covariance__,
        )
    ...
    def __get_state_and_F_matrix__(self, dt, state):
        ...
        if abs(omega) > 0.0001:
            F5 = np.array(
                [
                    [
                        (x[1] * (dt * cos_t_o * omega - sin_t_o) + x[3] * (-dt * sin_t_o * omega + 1 - cos_t_o))
                        / (omega**2)
                    ],
                    [-dt * sin_t_o * x[1] - dt * cos_t_o * x[3]],
                    [
                        (x[1] * (dt * sin_t_o * omega - 1 + cos_t_o) + x[3] * (dt * cos_t_o * omega - sin_t_o))
                        / (omega**2)
                    ],
